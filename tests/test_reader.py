...
async def test_reader_vector():
    ...
    async with PMTilesReader(VECTOR_PMTILES, store=store) as src:
        assert src.header
...
async def test_reader_raster():
    ...
    async with PMTilesReader(RASTER_PMTILES, store=store) as src:
        assert src.header
...
async def test_reader_bad_spec():
    ...
    with pytest.raises(AssertionError):
        async with PMTilesReader(V2_PMTILES, store=store) as src:
            pass
